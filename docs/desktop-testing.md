...
> 当前完整窗口记录以 macOS Apple Silicon 为基线。Windows x86_64 的移植顺序、平台专项检查和真实设备验收矩阵见 `docs/windows-porting.md`；Windows 候选版本还必须复用本文中所有不依赖 macOS 专有行为的任务、翻译、编辑、导出和烧录场景。
...
当前 macOS 完整桌面配置为 `tauri.macos-complete.conf.json`，在 FFmpeg、FFprobe 和 whisper.cpp 之外打包 `llama-server`。它使用正式 bundle identifier 和系统应用数据目录；Hy-MT2 GGUF 继续按需下载到设置页显示的 `models/` 目录，不嵌入 App。完成稳定桌面改动后运行 `./scripts/install-current-macos-app.sh`，只覆盖 `/Applications/Atogaki.app`，不生成 Test App 或日常测试 DMG。Windows runtime 仍需独立构建与实测。

打包后的真实窗口回归使用 Tauri CLI；`beforeBuildCommand` 显式把工作目录设置为 `../ui` 后执行 `npm run build`，避免调用位置改变时重复拼接前端路径。正式本地 App 由 `install-current-macos-app.sh` 使用完整 macOS 配置构建、核对四个 sidecar 与签名后替换 `/Applications/Atogaki.app`。配置声明最低 macOS 12.0，与 sidecar 的 deployment target 一致。
...
模型选择器会优先打开 `ATOGAKI_WHISPER_MODEL` 或 `ATOGAKI_VAD_MODEL` 所在目录；未配置时，如果 `~/Models` 存在则从该目录打开。启动环境中配置的模型路径会自动填入；若 Whisper 模型所在目录包含文件名带 `silero` 的 `.bin`，也会自动选择首个候选 VAD 模型。所有路径输入框都允许直接粘贴完整路径，作为原生文件面板不可用时的降级方式。设置页下载可使用用户填写的 HTTPS 镜像并回退 Hugging Face 官方源；文件先进入应用数据目录 `models/` 下的 `.part`，合法的未完成文件跨重启保留。再次下载时只有服务器返回与本地边界一致的 HTTP Range／Content-Range 才会继续追加；服务器忽略 Range、边界冲突或摘要失败时必须安全重启或拒绝安装。完整文件通过固定 SHA-256 后才原子安装。
...
3. 将翻译切换为“关闭”并保存，工作区翻译按钮应立即反映未配置状态，无需重启。分别切换 DeepL（传统翻译 API）、DeepSeek（LLM API）和 OpenAI-compatible（高级），确认模型／Base URL／风格字段按 provider 显示；Key 输入框不得回显既有值，SQLite 和任务目录中不得出现 Key。点击“检查所选 Key”时只读取当前 provider 的系统凭据条目，不调用翻译 API；存在、不存在和拒绝访问都要给出明确结果，同一进程后续翻译不得再次读取。真实 Key 写入测试只在明确允许修改本机系统凭据时执行。
   正式 macOS 本地 App 已包含 `llama-server`。安装 1.8B 与 7B 后应只有一个显示“当前使用”；点击另一档“设为当前”时必须先完成 SHA-256 校验，设置摘要和实际翻译使用的模型名随之更新。第一次翻译前不应出现 `llama-server` 进程；开始翻译后只监听 loopback，切走 provider 或退出 App 后不应遗留进程。
...
23. 在“导出成品 → 字体、样式与预览”确认左侧预览始终可见、右侧属性独立滚动，原文／译文分别读取任务默认值。本机字体浏览器应过滤点号开头的系统内部 family，显示全部候选而不是截断到 80 项，并在面板内独立滚动；搜索、清空搜索和反复打开期间背景保持不透明，说明文字完整换行，字体样张使用任务实际文字。选择不含任务字符的字体时应显示缺字／fallback 警告。修改字体、字号、颜色、描边、背景、位置或边距并停止操作约 700 毫秒后，应在不保存草稿的前提下自动显示当前播放位置附近的实际视频帧；连续修改期间不得并发堆积渲染，纯音频任务显示中性背景。“立即刷新”仍应可手动触发。展开日志可查看 libass 实际字体选择。
24. 明确保存任务样式后分别导出双语 ASS 和烧录视频，确认二者与预览使用相同样式；关闭并重开 App 后样式仍存在。导出 SRT 时确认界面已说明样式不会保留。另一台缺少所选字体的机器应保留请求的 family 名并显示替代／缺字提示，而不是静默改写任务设置。
25. 修改原文时点击“修正加入词表”，缩短误识别和规范写法后保存；词表管理器中应出现该规则并保留任务的源语言。
26. 修改译文并保存；“待重译”应清除，并显示译文已编辑。
27. 点击“翻译本段”；译文应写入 SQLite，刷新或重启后仍然存在。
28. 点击“全部翻译／重译”，确认覆盖提示；全部译文应一次性更新。当前 provider 没有可用 Key 时应保留完整错误提示且不得局部写入字幕。成功后“翻译与词表”应立即显示最近 provider、返回模型、批次段数、可得 token 与完成时间。
29. 使用 DeepSeek 或自定义 OpenAI-compatible 端点翻译包含任务保护词的片段；响应必须按稳定段 ID 写回，专名占位符完整恢复。模拟空内容、无效 JSON、重复／遗漏 ID 时应在一次自动重试后失败，并且 SQLite 字幕保持原状。
30. 修改原文但保留旧译文并保存；导出应拒绝，并提示先处理待重译字幕。
31. 重译后点击“导出字幕…”，选择一个目录；日语任务应生成以任务显示名称为前缀的 `.ja.srt`、`.zh-Hans.srt`、`.bilingual.srt` 和 `.bilingual.ass`，内容来自 SQLite 人工编辑后的状态。英语任务的原文文件应改为 `.en.srt`。
32. 再次导出到同一目录，应列出冲突文件并要求确认；取消后原文件保持不变，确认后才覆盖。导出成功后点击“在 Finder 中显示”，应选中双语 ASS 文件。
33. 点击“导出带字幕视频…”，确认能力面板显示 App Bundle 内的 `ffmpeg` 绝对路径、libass、VideoToolbox 和 MPEG-4 回退。分别选择双语、仅译文或仅原文以及 MP4 输出位置；对低码率源视频，VideoToolbox H.264 的目标码率应为源视频加约 20% 余量，而不是固定高码率。MPEG-4 回退编码效率较低，应记录实际大小与质量，不能假定同一比例。
34. 提交后进度应增加；关闭弹窗不影响烧录。取消后记录应变为“已取消”，目标目录不应留下 `.partial.mp4`。
35. 完成后记录应显示最终编码器和音频处理方式；VideoToolbox 运行时失败时，应显示原始回退原因与 “MPEG-4 软件编码”。点击“在 Finder 中显示”应选中最终 MP4。若 MPEG-4 也失败，任务才失败，并保留本次 ASS 快照和错误记录。
36. 任务目录 `renders/` 应保存本次不可变 ASS 快照；烧录期间继续编辑 SQLite 字幕只影响下次提交。
37. 将原媒体临时移走后重新打开任务；若任务目录已有 `audio.wav`，应回退到音频，同时明确提示原媒体缺失并显示“重新定位原媒体”。选择移动后的同一文件，确认画面、收听、字幕编辑和视频烧录恢复，已编辑字幕不变。
38. 进入顶层“收听”，打开一个完整翻译任务；字幕应以连续全文段落显示，当前句仍随播放高亮，时间码可独立跳转。拖选一个字幕段内的词、短语或语法表达时不得触发 seek，选区操作条应显示实际文本；跨段选择不应允许保存。
39. 分别将选区保存为词／语法和用段落按钮收藏整句；进入顶层“学习”后应看到原文、完整来源例句、任务快照和段时间。整句应使用当前字幕译文预填释义，词／语法在没有词典 provider 时仍可保存，并允许手工填写、修改和清空释义。
40. 再次从另一个字幕来源收藏同语言、同类型、同文本；学习区应保持一个条目并增加来源数。删除学习条目后刷新不应恢复；在隔离数据目录中删除来源任务后，学习条目与收藏时例句快照仍应存在，但返回原媒体按钮不可再使用。
41. 从仍有任务引用的学习条目点击“播放来源”；应用应切到收听区、打开对应任务、定位到该字幕段开始并播放。原媒体已移动时应沿用任务的音频回退／重新定位提示，不生成或持久化词级音频片段。
42. 学习列表应只显示简明译义和一条例句，不直接展开完整词典。点击词／语法条目的“查看词典”：英语标签应为“简明／ECDICT 英中／Merriam-Webster”，不得出现 Collins 或 Cambridge；日语应为“简明／JMdict／Tomoshi”。未配置 API 显示“未配置”，未下载离线包显示“未下载”，且不得访问网络、系统凭据或自动开始下载。整句收藏只应出现“简明”。
43. 打开设置的“学习词典”：应显示正式 `dictionaries/` 目录，以及 JMdict、Tomoshi、ECDICT 基础英中三张包卡片的语言、版本、大小、许可和署名，不得出现 FreeDict 下载入口；仅打开设置不得发起包下载或读取系统凭据。点击一个包后应复用当前代理、显示解析／下载进度，摘要错误或中断不得留下可用安装文件，成功后显示版本与正式路径；重启后仍显示已安装。真实大包由用户设备执行，自动化测试不得为了取摘要把词典包下载到临时隔离区。
44. 下载 ECDICT 后首次查询 `gadget` 与 `gadgets`：首次应明确停留在“正在查询”并完成版本化索引，两个查询都展示实际词头及中文释义；记录索引耗时、峰值内存和派生 SQLite 大小。随后重启 App 再查同词，不应重新扫描 CSV。ECDICT 查询失败不得阻止切换 Merriam-Webster。
45. 为 Merriam-Webster 输入测试 Key，保存后输入框必须清空。SQLite 只能出现 `dictionary.merriam_webster_key_saved=true` 标记，不得出现 Key；设置和学习详情不得出现 Collins 或 Cambridge。升级不得为了清理历史状态而在启动时读取或删除历史 Keychain 项目。“检查”可触发一次系统凭据读取但不调用 API，“删除”只删除 Merriam-Webster 来源。
46. 在测试数据库注入两个 provider 结果后重新打开详情；切换标签应分别保留命中词形、读音／音标、词性、义项、例句和署名，不得合并或互相覆盖。关闭详情后，学习列表布局和用户简明译义不应改变。
47. 在至少三个任务的工作台中，先用六点手柄、再用任务卡标题区域将第一项拖到第三项之后；按下时应立即显示“正在调整任务顺序”，移动超过阈值后卡片跟随命中位置，松开后显示保存成功且不得误入任务。刷新、重启和进入收听区后顺序应一致，新建任务应暂时置顶。实体鼠标与实体触控板至少各验证一次；自动化坐标拖动只作为回归，不能替代物理输入验收。聚焦六点按钮后用上下方向键也应完成相同持久化排序，重命名／重试／删除按钮不得触发拖动。
