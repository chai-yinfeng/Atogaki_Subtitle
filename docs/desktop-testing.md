...
36. 进入顶层“收听”，打开一个完整翻译任务；字幕应以连续全文段落显示，当前句仍随播放高亮，时间码可独立跳转。拖选一个字幕段内的词、短语或语法表达时不得触发 seek，选区操作条应显示实际文本；跨段选择不应允许保存。
37. 分别将选区保存为词／语法和用段落按钮收藏整句；进入顶层“学习”后应看到原文、完整来源例句、任务快照和段时间。整句应使用当前字幕译文预填释义，词／语法在没有词典 provider 时仍可保存，并允许手工填写、修改和清空释义。
38. 再次从另一个字幕来源收藏同语言、同类型、同文本；学习区应保持一个条目并增加来源数。删除学习条目后刷新不应恢复；在隔离数据目录中删除来源任务后，学习条目与收藏时例句快照仍应存在，但返回原媒体按钮不可再使用。
39. 从仍有任务引用的学习条目点击“播放来源”；应用应切到收听区、打开对应任务、定位到该字幕段开始并播放。原媒体已移动时应沿用任务的音频回退／重新定位提示，不生成或持久化词级音频片段。
40. 学习列表应只显示简明译义和一条例句，不直接展开完整词典。点击词／语法条目的“查看词典”：英语标签应为“简明／Cambridge／Collins／Merriam-Webster”，日语应为“简明／JMdict／Tomoshi”；未配置 API 显示“未配置”，未下载离线包显示“未下载”，且不得访问网络、系统凭据或自动开始下载。整句收藏只应出现“简明”。
41. 打开设置的“学习词典”：应显示正式 `dictionaries/` 目录，以及 JMdict、Tomoshi、ECDICT 基础英中、FreeDict 英中四张包卡片的语言、版本、大小、许可和署名；仅打开设置不得发起包下载或读取系统凭据。点击一个包后应复用当前代理、显示解析／下载进度，摘要错误或中断不得留下可用安装文件，成功后显示版本与正式路径；重启后仍显示已安装。真实大包由用户设备执行，自动化测试不得为了取摘要把词典包下载到临时隔离区。
42. 下载 ECDICT 后首次查询 `gadget` 与 `gadgets`：首次应明确停留在“正在查询”并完成版本化索引，两个查询都展示实际词头及中文释义；记录索引耗时、峰值内存和派生 SQLite 大小。随后重启 App 再查同词，不应重新扫描 CSV。ECDICT 查询失败不得阻止切换 FreeDict／Merriam-Webster；确认 ECDICT 可用前不删除 FreeDict 包。
42. 分别为 Cambridge、Collins、Merriam-Webster 输入测试 Key，保存后输入框必须清空，三家状态互不覆盖。SQLite 只能出现 `dictionary.*_key_saved=true` 标记，不得出现 Key；“检查”可触发一次系统凭据读取但不调用 API，“删除”只删除所选来源。没有明确授权时不得写入或删除开发机真实 Keychain 项目。
43. 在测试数据库注入两个 provider 结果后重新打开详情；切换标签应分别保留命中词形、读音／音标、词性、义项、例句和署名，不得合并或互相覆盖。关闭详情后，学习列表布局和用户简明译义不应改变。
