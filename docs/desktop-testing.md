...
11. 点击“打开悬浮字幕”；独立窗口必须能通过明确的标题栏区域拖动、通过可见的边角控制缩放。等比例放大窗口时，标题、语言标签和两行字幕应同步放大；缩小时应同步缩小，长字幕不得被裁掉。播放跨段、跳转和当前字幕编辑后应同步原文与译文。以三指切换至另一普通桌面后，窗口应仍然显示；关闭悬浮窗或返回任务列表后，它不应继续遮挡其他应用。全屏空间覆盖需要单独记录，不能由“置顶”一词默认推断。
12. 修改原文并保存；刷新或重启应用后修改仍应存在。已有译文时，原文修改应标记“待重译”。
13. 在工作区选择同语言、带修正规则的词表并预览；确认后只修改匹配段，已有译文应标记为待重译。其他语言的词表不应出现在候选中。
14. 修改原文时点击“修正加入词表”，缩短误识别和规范写法后保存；词表管理器中应出现该规则并保留任务的源语言。
15. 修改译文并保存；“待重译”应清除，并显示译文已编辑。
16. 点击“翻译本段”；译文应写入 SQLite，刷新或重启后仍然存在。
17. 点击“全部翻译／重译”，确认覆盖提示；全部译文应一次性更新。没有 `DEEPL_AUTH_KEY` 时按钮应禁用并显示配置提示。
