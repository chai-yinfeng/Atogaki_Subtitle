...
23. 在“导出成品 → 字体、样式与预览”确认左侧预览始终可见、右侧属性独立滚动，原文／译文分别读取任务默认值，本机字体族可检索；选择不含任务字符的字体时应显示缺字／fallback 警告。修改字体、字号、颜色、描边、背景、位置或边距并停止操作约 700 毫秒后，应在不保存草稿的前提下自动显示当前播放位置附近的实际视频帧；连续修改期间不得并发堆积渲染，纯音频任务显示中性背景。“立即刷新”仍应可手动触发。展开日志可查看 libass 实际字体选择。
