...
41. 打开设置的“学习词典”：应显示正式 `dictionaries/` 目录，以及 JMdict、Tomoshi、ECDICT 基础英中三张包卡片的语言、版本、大小、许可和署名，不得出现 FreeDict 下载入口；仅打开设置不得发起包下载或读取系统凭据。点击一个包后应复用当前代理、显示解析／下载进度，摘要错误或中断不得留下可用安装文件，成功后显示版本与正式路径；重启后仍显示已安装。真实大包由用户设备执行，自动化测试不得为了取摘要把词典包下载到临时隔离区。
42. 下载 ECDICT 后首次查询 `gadget` 与 `gadgets`：首次应明确停留在“正在查询”并完成版本化索引，两个查询都展示实际词头及中文释义；记录索引耗时、峰值内存和派生 SQLite 大小。随后重启 App 再查同词，不应重新扫描 CSV。ECDICT 查询失败不得阻止切换 Merriam-Webster。
