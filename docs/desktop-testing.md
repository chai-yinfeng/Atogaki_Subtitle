...
9. 点击任务进入工作区；媒体应能播放，字幕段按时间排序。
   - 顶部应只显示“字幕校对／翻译与词表／导出成品”三个一级模块；返回、任务状态、重新读取和操作反馈始终位于模块外。鼠标点击及左右方向键都应切换模块并正确更新选中状态。
   - 在字幕校对中创建未保存草稿，切到另外两个模块后再返回；文字、时间和按钮状态必须保留。切换模块不得强制暂停媒体、重新读取任务或丢失当前播放位置。
...
14. 在原文中放置光标后点击“从原文光标拆分”；预览应给出可编辑的左右原文、译文和位于原时间范围内的分界。确认后段数加一、右段获得新 ID、后续索引连续；任一侧译文留空时两段都应显示未翻译，不得保留部分译文。
15. 点击“与下一段合并”；只允许相邻段，预览应使用完整时间并集以及按语言连接的文字。任一段没有译文时合并结果应为未翻译；任一旧译文过期时结果继续待重译。确认后段数减一且保留左段 ID。
16. 连续执行拆分和合并后逐步点击时间轴标题旁的“撤销上次拆分／合并”；每次应原子恢复段数、ID、顺序、文字、译文、时间和编辑标记。结构操作后修改或翻译字幕时，陈旧的结构撤销必须禁用。重新读取并重启 App 后结构仍存在，但会话内撤销按钮不可用。
...
26. 点击“全部翻译／重译”，确认覆盖提示；全部译文应一次性更新。没有 `DEEPL_AUTH_KEY` 时按钮应禁用并显示配置提示。
27. 修改原文但保留旧译文并保存；导出应拒绝，并提示先处理待重译字幕。
28. 重译后点击“导出字幕…”，选择一个目录；日语任务应生成以任务显示名称为前缀的 `.ja.srt`、`.zh-Hans.srt`、`.bilingual.srt` 和 `.bilingual.ass`，内容来自 SQLite 人工编辑后的状态。英语任务的原文文件应改为 `.en.srt`。
29. 再次导出到同一目录，应列出冲突文件并要求确认；取消后原文件保持不变，确认后才覆盖。导出成功后点击“在 Finder 中显示”，应选中双语 ASS 文件。
30. 点击“导出带字幕视频…”，确认能力面板显示 App Bundle 内的 `ffmpeg` 绝对路径、libass、VideoToolbox 和 MPEG-4 回退。分别选择双语、仅译文或仅原文以及 MP4 输出位置；对低码率源视频，VideoToolbox H.264 的目标码率应为源视频加约 20% 余量，而不是固定高码率。MPEG-4 回退编码效率较低，应记录实际大小与质量，不能假定同一比例。
31. 提交后进度应增加；关闭弹窗不影响烧录。取消后记录应变为“已取消”，目标目录不应留下 `.partial.mp4`。
32. 完成后记录应显示最终编码器和音频处理方式；VideoToolbox 运行时失败时，应显示原始回退原因与 “MPEG-4 软件编码”。点击“在 Finder 中显示”应选中最终 MP4。若 MPEG-4 也失败，任务才失败，并保留本次 ASS 快照和错误记录。
33. 任务目录 `renders/` 应保存本次不可变 ASS 快照；烧录期间继续编辑 SQLite 字幕只影响下次提交。
34. 将原媒体临时移走后重新打开任务，应显示可操作错误；若任务目录已有 `audio.wav`，应回退到音频。
