...
11. 点击“打开悬浮字幕”；Atogaki 的 Dock 图标和菜单栏应在悬浮模式期间暂时隐藏。独立面板必须能通过原生标题栏拖动、通过原生窗口边缘缩放。等比例放大窗口时，标题、语言标签和两行字幕应同步放大；缩小时应同步缩小，长字幕不得被裁掉。播放跨段、跳转和当前字幕编辑后应同步原文与译文。以三指切换至另一普通桌面或全屏应用 Space 后，面板都应仍然显示。
