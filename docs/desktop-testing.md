...
2. 打开“管理词表”，新建或编辑词表；目标词留空的提示词和带目标词的修正规则在重启后都应保留。
3. 新建任务时选择词表；任务详情应显示所用词表，任务目录应包含 `recognition-glossary.txt` 快照。之后修改原词表不应改变该快照。
4. 应用保持响应，任务失败时卡片应显示 `failed` 和错误信息；成功后显示 `done`。
5. 点击任务进入工作区；媒体应能播放，字幕段按时间排序。
6. 播放时当前句应高亮；点击时间码应跳转并继续播放。
7. 修改日文并保存；刷新或重启应用后修改仍应存在。已有中文时，日文修改应标记“待重译”。
8. 在工作区选择带修正规则的词表并预览；确认后只修改匹配段，已有中文应标记为待重译。
9. 修改日文时点击“修正加入词表”，缩短误识别和规范写法后保存；词表管理器中应出现该规则。
10. 修改中文并保存；“待重译”应清除，并显示中文已编辑。
11. 点击“翻译本段”；译文应写入 SQLite，刷新或重启后仍然存在。
12. 点击“全部翻译／重译”，确认覆盖提示；全部译文应一次性更新。没有 `DEEPL_AUTH_KEY` 时按钮应禁用并显示配置提示。
13. 修改日文但保留旧中文并保存；导出应拒绝，并提示先处理待重译字幕。
14. 重译后点击“从 SQLite 导出 SRT／ASS”；任务目录中的 `ja.srt`、`zh.srt`、`bilingual.srt` 和 `bilingual.ass` 应包含 SQLite 人工编辑后的内容。
15. 将原媒体临时移走后重新打开任务，应显示可操作错误；若任务目录已有 `audio.wav`，应回退到音频。
