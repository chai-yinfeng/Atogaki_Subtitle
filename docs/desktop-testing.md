...
41. 打开设置的“学习词典”：应显示正式 `dictionaries/` 目录，以及 JMdict、Tomoshi、FreeDict 英中三张包卡片的语言、版本、大小、许可和署名；仅打开设置不得发起包下载或读取系统凭据。点击一个包后应复用当前代理、显示解析／下载进度，摘要错误或中断不得留下可用安装文件，成功后显示版本与正式路径；重启后仍显示已安装。真实大包由用户设备执行，自动化测试不得为了取摘要把词典包下载到临时隔离区。
