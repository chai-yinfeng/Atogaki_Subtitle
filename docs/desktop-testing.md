...
42. 学习列表应只显示简明译义和一条例句，不直接展开完整词典。点击词／语法条目的“查看词典”：英语标签应为“简明／ECDICT 英中／Collins／Merriam-Webster”，不得再出现 Cambridge；日语应为“简明／JMdict／Tomoshi”。未配置 API 显示“未配置”，未下载离线包显示“未下载”，且不得访问网络、系统凭据或自动开始下载。整句收藏只应出现“简明”。
...
45. 分别为 Collins、Merriam-Webster 输入测试 Key，保存后输入框必须清空，两家状态互不覆盖。SQLite 只能出现对应的 `dictionary.*_key_saved=true` 标记，不得出现 Key；旧 `dictionary.cambridge_key_saved` 标记在迁移后应消失。设置和学习详情不得出现 Cambridge；升级不得为了清理历史状态而在启动时读取或删除开发机 Keychain 项目。“检查”可触发一次所选来源的系统凭据读取但不调用 API，“删除”只删除所选来源。
