...
23. 在“导出成品 → 字体、样式与预览”确认原文／译文分别读取任务默认值，本机字体族可检索；选择不含任务字符的字体时应显示缺字／fallback 警告。修改字号、颜色、描边、背景、位置或边距后点击“生成真实预览”，应在不保存草稿的前提下显示当前播放位置附近的实际视频帧；纯音频任务显示中性背景。展开日志可查看 libass 实际字体选择。
...
42. 学习列表应只显示简明译义和一条例句，不直接展开完整词典。点击词／语法条目的“查看词典”：英语标签应为“简明／Cambridge／Collins／Merriam-Webster”，日语应为“简明／JMdict／Tomoshi”；未配置 API 显示“未配置”，未下载离线包显示“未下载”，且不得访问网络、系统凭据或自动开始下载。整句收藏只应出现“简明”。
...
45. 分别为 Cambridge、Collins、Merriam-Webster 输入测试 Key，保存后输入框必须清空，三家状态互不覆盖。SQLite 只能出现 `dictionary.*_key_saved=true` 标记，不得出现 Key；“检查”可触发一次系统凭据读取但不调用 API，“删除”只删除所选来源。没有明确授权时不得写入或删除开发机真实 Keychain 项目。
