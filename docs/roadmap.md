...
- [x] 桌面端提供日语、英语源语言选择，首批目标语言固定为简体中文。
- [x] 为 Whisper 和 DeepL 分别映射规范语言代码，并拒绝 provider 不支持的组合。
- [x] 为词表记录源语言，只允许任务使用同语言词表；现有词表保持为日语。
- [x] 保留日语回归并增加英语到简体中文的任务、迁移和导出自动化测试。

**验收：** 同一个安装可以分别创建日语和英语节目任务；重启后仍使用正确语言对完成转写、翻译、编辑、导出与烧录。
...
- [ ] 悬浮字幕窗口：独立显示原文／译文，支持可靠拖动、可见缩放控制，以及跨普通 macOS 桌面的置顶显示。
...
- 当前识别与翻译选项已可配置；UI 暴露日语或英语到简体中文，并将语言对持久化到任务、SQLite、词表和导出。日语与英语真实节目均已在打包窗口中完成识别、DeepL 翻译、编辑、导出与烧录回归；新增语言仍需分别验证 Whisper、provider 与字幕排版质量。
