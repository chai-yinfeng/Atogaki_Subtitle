...
- [x] 烤肉区人工编辑独立字幕块，支持波形定位、10 ms 吸附、主体移动和单侧边缘修剪；空白合法，同轨不得新增重叠，并在快照同步后保留修改。普通任务详情只保留文字粗修。
- [x] 为原文、译文和时间轴提供放弃未保存草稿，以及带指纹保护的会话内多步保存撤销。
- [x] 主工作区与获得焦点的悬浮字幕共享键盘映射；悬浮字幕同时提供鼠标播放、跳转和调速控制。
- [x] 烤肉区播放头 Cut 与相接字幕 Join：采用冻结 SQLite 段结构的原子编辑；Cut 在没有文本光标时复制文字到两块，Join 不跨越合法空白，并提供会话内结构撤销。
...
- [x] 建立顶层“烤肉”工作区。第一里程碑提供任务切换、可缓存波形、时间尺、字幕块、播放头、缩放和 10／100 ms 精确定位；离开后暂停媒体，返回时恢复当前任务与播放位置。后续里程碑再接入拖动写库与连续打轴。
