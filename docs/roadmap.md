...
- [ ] 建立独立于识别纠错词表的学习数据模型；收藏项保留任务、字幕段、播放时间和收藏时文本快照。
- [ ] 支持从字幕原文选择词、短语或语法片段，连同完整例句上下文保存；整句收藏作为无需文本选择的快捷入口。
- [ ] 为收藏项保存或重新查询译义，并保留实际 provider／来源；查询失败时仍允许先保存原文与上下文、稍后补充。
- [ ] 第一版音频关联复用原任务媒体与字幕段时间范围，不把媒体二进制写进 SQLite；精确到词的裁剪在取得可靠词级时间或手动边界后再扩展。
- [ ] 在任务字幕卡和收听区提供收藏入口，并允许取消收藏。优先评估把现有收听字幕流升级为可选择文本的全文阅读模式，以当前播放句高亮和点击定位复用同一播放器，而不是建立第三套重复预览页。
- [ ] 建立与工作台、收听同级的“学习”区域，提供基础检索和按语言／任务筛选。
- [ ] 从收藏项回到原任务、字幕段和播放时间；原媒体缺失时复用任务的重新定位能力。
- [ ] 用真实收藏与回看行为验证学习闭环后，再决定内建复习、间隔重复、复杂标签或 Anki 导出的顺序。

第一版不同时引入自动生成长篇 AI 语法讲解、间隔重复、复杂标签体系或自定义 Anki 模板。学习数据既保留对当前字幕主数据的引用，也保留收藏时快照，避免后续字幕修改让旧学习记录失去上下文。全文阅读交互、收藏数据与学习列表可以分里程碑交付；在完成选择手势原型前，不把具体页面布局写成不可逆架构决定。
