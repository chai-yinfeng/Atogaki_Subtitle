...
当前主线顺序为 **Windows x86_64 基础发行 → 窄版重新识别 → 学习区 MVP**。字幕编辑效率可按真实打轴频率穿插小步补齐，但不继续扩大 Windows 移植前的 macOS 专属功能面。实时辅助保持远期；多人字幕轨道和样式渲染先作为需要真实需求与产品定位复核的候选扩展。
