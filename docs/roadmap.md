...
Windows x86_64 基础发行已经以 alpha 进入扩测，学习区已完成可用基线，当前 macOS Apple Silicon 主线是 **任务级字幕样式与 libass 真实预览 → 窄版重新识别**。学习区检索、筛选和复习增强按真实使用回补；字幕编辑效率可按真实打轴频率穿插小步补齐。Windows 已知问题在稳定候选节点集中修复，不与日常 macOS 功能开发形成长期分支；实时辅助和多人字幕轨道保持后续阶段。
