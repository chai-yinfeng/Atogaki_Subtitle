...
3. macOS Apple Silicon 继续作为日常开发与发布质量基线；Windows 已恢复明确的 NSIS current-user／中英配置，并把 alpha.6 后共享功能推进到同一源码基线。先取得原生编译结果，再决定是否生成不发布的安装候选并集中验证学习区、词典、字体样式、任务排序、可恢复翻译、局部重新识别、长视频 Range 播放和导出质量档；悬浮字幕异常暂缓且不得标记为可用。
...
- [x] 在字幕编辑中对用户明确填写的完整字幕范围执行隔离前文上下文的局部重新识别，先并排预览、再由用户确认原子替换；不自动扩展范围或覆盖预览期间发生的新编辑。
...
2026-09-07 《響け！ユーフォニアム》广播节目暴露 Whisper 在真实尾段把短音节反馈成长循环的问题。局部修复入口因此提前进入字幕编辑：复用任务冻结的模型、语言、VAD 和词表，仅显式把文字历史上下文设为隔离；选区边界切穿字幕时直接拒绝，由用户选择是否扩展到完整字幕块。候选结果停留在会话预览中，确认时再次校验原字幕快照并只替换所选范围。这个窄版修复不取代后续“派生完整对照任务”。
...
- 2026-09-07 YouTube LIVE 全量翻译在前 74 个批次（888 段）成功后，第 75 批的 348 ms 残片 `す。` 连续两次收到 DeepSeek 空译文。当前“所有批次成功后一次性写入”语义使已经成功并记录 token 的 888 段仍未进入字幕工作区。短期可在字幕编辑中把残片与前段 Join 后重试；是否改为批次检查点／从失败批次继续，会改变既有整次原子语义和失败后的费用预期，需要单独决策，不能作为无提示 fallback。
