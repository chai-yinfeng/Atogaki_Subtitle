...
- [x] 烤肉区人工编辑字幕边界，支持波形定位、10 ms 吸附、相邻边界联动、空隙／重叠提示，并在快照同步后保留修改；普通任务详情只保留文字粗修。
- [x] 为原文、译文和时间轴提供放弃未保存草稿，以及带指纹保护的会话内多步保存撤销。
- [x] 主工作区与获得焦点的悬浮字幕共享键盘映射；悬浮字幕同时提供鼠标播放、跳转和调速控制。
- [x] 烤肉区字幕拆分与相邻合并：采用冻结 SQLite 段结构的原子编辑，提供确认预览和会话内结构撤销。
...
- [x] 相邻时间边界联动、波形上的字幕块／边界拖动、10 ms 吸附与重叠／空隙提示；保存采用带当前快照校验的 SQLite 原子操作，拒绝夹带文字或结构变化。
