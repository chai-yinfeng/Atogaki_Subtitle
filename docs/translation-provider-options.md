...
_调研日期：2026-08-11；产品选择与实现状态更新：2026-08-22。免费额度、模型名和服务条款会变化，发布前必须重新核对官方页面。_
...
- 2026-08-16 已使用同一真实节目片段完成 DeepL／DeepSeek A/B：DeepSeek 的 60/60 稳定段 ID 通过校验并原子写入，跨句承接和口语简洁度在抽样中通常更自然，专名保护有效；个别主语补全仍需人工判断。具体模型、token 和测试成本记录见 `docs/roadmap.md` 的 1.3 阶段。模型、价格或响应协议变化后仍需重新验证，不能把这次结果永久外推到所有节目。
