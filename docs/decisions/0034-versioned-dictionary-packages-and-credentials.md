...
- 当前 Merriam-Webster 使用 `dictionary:merriam-webster` 凭据 ID 写入平台系统凭据库。SQLite 只保存“曾由本 App 成功保存”的布尔标记；打开设置和列出状态不读取 Keychain，只有用户主动保存、删除或检查时访问。Collins 于 2026-09-16 退出产品路径；为避免启动时产生系统授权提示，升级不隐式读取或删除可能存在的 Collins／Cambridge 历史 Keychain 条目。
