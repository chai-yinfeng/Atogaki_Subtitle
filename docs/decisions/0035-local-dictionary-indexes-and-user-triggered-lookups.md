...
用户已经在正式应用数据目录安装 JMdict、Tomoshi 和当时仍在评估的 FreeDict，并为 Merriam-Webster 保存 API Key。离线包格式不同：JMdict 是约 117 MiB 的 JSON，Tomoshi 是 Zstandard 压缩的 SQLite，FreeDict 是 StarDict。逐次扫描归档会让学习区查询不可用；把上游包转换后覆盖原件又会丢失版本与校验依据。2026-08-25 FreeDict 已由后续 ECDICT 决策取代，以下相关条款作为历史记录保留，不再代表当前产品目录。
...
- 原始归档继续作为已校验的来源事实保留。当前 JMdict 与决策 0036 引入的 ECDICT 基础 CSV 在第一次查询或包版本变化后生成 `dictionaries/atogaki-dictionary-index.sqlite`；索引只保存查询所需词形、词头、读音和规范化义项，不复制无关字段。历史 FreeDict 索引行可以留在可再生成的派生库中，但已无产品入口。
...
- Collins 最初保留 provider 和凭据入口，但始终没有可验证账户或查询实现；2026-09-16 已删除 provider、来源标签和凭据入口。Cambridge 因官方确认当前 API 不可用，也不在产品路径中。
